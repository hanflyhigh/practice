...
command! -nargs=0 -bar SrcExplUpdateTags
    \ call g:SrcExpl_UpdateTags()

command! -nargs=0 -bar SrcExplUpdateCscope
    \ call g:SrcExpl_UpdateCscope()
...
" User interface to control if update the 'cscope' file when loading
" the Source Explorer, 0 for false, others for true
if !exists('g:SrcExpl_isUpdateCscope')
    let g:SrcExpl_isUpdateCscope = 1
endif

" User interface to create a 'cscope' file using exact ctags
" utility, 'cscope -Rbkq .' as default
if !exists('g:SrcExpl_updateCscopeCmd')
    let g:SrcExpl_updateCscopeCmd = 'cscope -Rbkq'
endif


" User interface to update tags file artificially
if !exists('g:SrcExpl_updateCscopeKey')
    let g:SrcExpl_updateCscopeKey = '<F6>'
endif
...
function! g:SrcExpl_UpdateTags()
	...
	" Ask user if or not update a tags file
	echohl Question
				\ | let l:tmp = <SID>SrcExpl_GetInput("\nSrcExpl: "
				\ . "Update tags in the current directory now? (y)es/(n)o?") |
	echohl None
	" They do
	if l:tmp == "y" || l:tmp == "yes"
...
" SrcExpl_UpdateCscope() {{{

" Update cscope file with the 'cscope' utility

function! g:SrcExpl_UpdateCscope()
	" Go to the current work directory
	" silent! exe "cd " . expand('%:p:h')

	" Ask user if or not create a cscope file
	echohl Question
				\ | let l:tmp = <SID>SrcExpl_GetInput("\nSrcExpl: "
				\ . "Create cscope in the current directory now? (y)es/(n)o?") |
	echohl None

	if l:tmp == "y" || l:tmp == "yes"
		" We tell user where we create a cscope file
		echohl Question
		echo "SrcExpl: Creating 'cscope' file in (". expand('%:p:h') . ")"
		echohl None
		" Call the external 'cscope' utility program
		"if filereadable("cscope.out")
		"	exe "!" . "rm cscope*"
		"	"execute like "cscope -Rbkq" in shell
		"	exe "!" . g:SrcExpl_updateCscopeCmd
		"	"execute like ":cscope reset" in vim
		"	exe 'silent! ' . 'cscope reset'
		"else
		"	"execute like "cscope -Rbkq" in shell
		"	exe "!" . g:SrcExpl_updateCscopeCmd
		"	"execute like ":cscope add cscope.out" in vim
		"	exe 'silent! ' . 'cscope add cscope.out'
		"endif	
		if filereadable("cscope.out")
			exe "!" . "rm cscope*"
		endif
		"execute like "cscope -Rbkq" in shell
		exe "!" . g:SrcExpl_updateCscopeCmd
		"execute like ":cscope kill -1" in vim,kill all connection
		exe 'silent! ' . 'cscope kill -1'
		"execute like "cscope add cscope.out"
		exe 'silent! ' . 'cscope add cscope.out'

		" Rejudge the Cscope file if existed
		if !filereadable("cscope.out")
			" Tell them what happened
			call <SID>SrcExpl_ReportErr("Execute 'cscope' utility program failed")
			return -1
		endif
	endif
	return 0
            ...
            if l:dict != {}
                ...
                if l:dict['filename'][0] == '.'
                    put! ='[File Path]: ' . l:path . l:dict['filename'][2:]
                        \ . ' ' . '[Ref]: ' . l:dict['cmd']
                        "\ . ' ' . '[Ex Command]: ' . l:dict['cmd'] "robinfan
                else
...
function! <SID>SrcExpl_CleanUp()
    ...
    " Unmap the update-cscope key
    if maparg(g:SrcExpl_updateCscopeKey, 'n') ==
        \ ":call g:SrcExpl_UpdateCscope()<CR>"
        "exe "nunmap " . g:SrcExpl_updateCscopeKey
    endif
...
function! <SID>SrcExpl_Init()
    ...
    "exe "set autochdir" "robinfan for tag reference dispaly root path.
    " Let Vim find the possible tags file
    "exe "set tags=tags;" "robinfan
	...
	if g:SrcExpl_isUpdateCscope != 0
        " Update the Cscope file right now
        if g:SrcExpl_UpdateCscope()
            return -2
        endif
    endif

	if g:SrcExpl_updateCscopeKey != ""
        "exe "nnoremap " . g:SrcExpl_updateCscopeKey .
        "    \ " :call g:SrcExpl_UpdateCscope()<CR>"
    endif
