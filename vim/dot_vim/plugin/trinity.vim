...
command! -nargs=0 -bar TrinityToggleMiniBufExplorer
    \ call <SID>Trinity_ToggleMiniBufExplorer()
...
"robinfan add for MiniBufExplorerexplorer
let s:minibuf_list_switch    = 0
let s:minibuf_list_title     = "-MiniBufExplorer-"
...
function! <SID>Trinity_InitTagList()
    ...
    "let g:Tlist_WinWidth = 23 "40 robinfan
    let g:Tlist_WinWidth = 40 "40 robinfan
    " Sort by the order
...
function! <SID>Trinity_InitSourceExplorer()
    ...
    if has("unix")
        let g:SrcExpl_winHeight = 8 "13 robinfan
    else
    ...
    " // Do not let the Source Explorer update the cscope file when opening          "
    "let g:SrcExpl_isUpdateCscope = 0
	" // update a cscope file                                                        "
    " let g:SrcExpl_updateCscopeCmd = "cscope -Rbkq"
    " // Set "<F6>" key for updating the cscope file artificially                   "
...
" Trinity_InitMiniBufExplorer() {{{

" Initialize the parameters of the 'MiniBufExplorer' plugin

function! <SID>Trinity_InitMiniBufExplorer()
...
" Trinity_ToggleMiniBufExplorer() {{{

" Initialize the parameters of the 'MiniBufExplorer' plugin

function! <SID>Trinity_ToggleMiniBufExplorer()
    ...
    call <SID>Trinity_UpdateStatus()
    if s:Trinity_switch == 0
        if s:minibuf_list_switch == 0
            "call <SID>Trinity_InitMiniBufExplorer()
            MiniBufExplorer
            let s:minibuf_list_switch = 1
        endif
    else
        if s:minibuf_list_switch == 1
            CMiniBufExplorer
            let s:minibuf_list_switch = 0
        else
            "call <SID>Trinity_InitMiniBufExplorer()
